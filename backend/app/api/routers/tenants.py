...
from app.models.domain import Tenant, User, PlanType, TenantConfig
from app.schemas.tenants import ProvisionTenantRequest, TenantConfigUpsert, ProvisionTenantResponse
from app.api.dependencies import require_tenant, superadmin_only, require_role, get_current_user_claims
    ...
    if is_superadmin:
        with bypass_rls():
            result = await db.execute(
                select(TenantConfig).where(TenantConfig.tenant_id == tenant_id, TenantConfig.key == data.key)
            )
