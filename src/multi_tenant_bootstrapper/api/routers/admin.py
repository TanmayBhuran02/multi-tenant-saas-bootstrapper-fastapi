"""Superadmin panel router — tenant listing and metrics."""
...
from multi_tenant_bootstrapper.models.domain import (
    Tenant, User, TenantConfig, FeatureFlag, TenantStatus, PlanType,
)
from multi_tenant_bootstrapper.schemas.admin import (
    TenantListResponse, GlobalMetricsResponse, TenantMetricsResponse,
)
from multi_tenant_bootstrapper.api.dependencies import superadmin_only
...
async def tenant_metrics(
    ...
    with bypass_rls():
        ...
        c_count = await db.execute(
            select(func.count(TenantConfig.id)).where(TenantConfig.tenant_id == tenant_id)
        )
